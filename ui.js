  ...
  render(profile, repos) {
    ...
    repos.forEach((repo) => {
      repoCards += `
			...
			<a href="${repo.html_url}" class="mb-3 fs-4">${repo.name}</a>
    ...
    <div class="container">
      <div class="row">
        <div class="col-sm-4">
          <figure>
            <img src="${profile.avatar_url}" alt="" style="width: 150px; height: 150px; object-fit: cover" class="rounded-circle"/>
          </figure>
          <h1 class="fs-3">${profile.login}</h1>
          <p>${profile.bio}</p>
          <a href="${profile.html_url}" class="btn btn-primary" target="_blank">View Profile</a>
          <div class="mb-3">
            <span class="badge bg-primary">Flowwing: ${profile.following}</span>
            <span class="badge bg-info">Repos: ${profile.public_repos}</span>
            <span class="badge bg-success">Followers: ${profile.followers}</span>
          </div>
          <ul class="list-group">
           <li class="list-group-item">Website: <a herf="${profile.blog}">${profile.blog}</a></li>
